...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0:00 | Open the site (`/`) | Heading "Collect the surprise. Produce only what's wanted.", the Astral Kin stage with the **93 of 100 claimed** sticker and the 20-block live bar | "Blind boxes today: bots buy the drop, factories guess demand. We fix both." |
| 0:08 | **Explore the drops** → the **Astral Kin** card | `/drops` with 7 cards (3 live, 4 coming soon; Naruto and Cyberpunk: Edgerunners carry "Concept partner drop — demo only, not licensed"), then "Astral Kin™" and "7 remaining" | "Every character's odds are on the page before you pay." → click **Play to unlock** |
| 0:18 | **Demo: win instantly** (or **Untimed lore challenge** → **Start lore challenge** and answer the three questions) | "Quest cleared." | "The game is free. Winning gives one slot for 15 minutes, a few tries a day, so bots can't farm slots." → click **Claim preorder slot** |
| 0:25 | **Open my box** (tap the cube) → swipe across the foil pack (or **Skip**) | Cube shakes, lid flies off, a foil pack rises; the swipe slices it along your stroke; "Eclipse Knight", "1 of 100 made in this drop", "You have a duplicate" | "The server drew position 94 of a shuffle we fingerprinted before sales opened — before any animation played." |
| 0:40 | **Keep it — go to checkout** → tick **I am 18 or older** → **Checkout** → tick **I understand these figures will be made just for me.** → **Yes, confirm & pay** | "Are you sure you want these made?", then (no Stripe keys) the simulated payment, then "Confirmed. You're on the production list." | "You see your pull before you pay, but declining returns it to the pool and your slot never re-rolls." |
| 0:55 | **Find a trade** (on the success page) → tick **Aurora Warden** → **Find my match** → **Accept exchange**                                                              | "Two kin. Two happy collectors." then "Exchange complete."                                                                          | "Duplicates swap before anything is made, only inside the same rarity, and both people must accept."                                             |
...
- **Concept drops.** `/drops/naruto` → **Play to unlock** → win → open → checkout shows the demo payment sheet ("This is a concept drop. No money is charged.") → **Complete demo payment**. Licensed themes never reach Stripe.
- **Physical figure by QR.** As **Admin**, open `/admin/qr-sheet` → **Generate codes** → **Print sheet**. As Alex, **My collection** → **Add more** → scan a printed code (or **Enter code manually**) → reveal with **Verified physical · #001 / 60**.
